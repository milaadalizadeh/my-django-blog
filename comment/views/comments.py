    ...
    def form_valid(self, form):
      ...
      #send email section 
        ...
        if author_email:
            email = EmailMessage(
                        "دیدگاه جدید",
                         "دیدگاه جدیدی برای مقاله «{}» که شما نویسنده آن هستید دریافت شد :\n{}{}".format(article, current_site, reverse('myblog:post', kwargs={'slug': article.slug})),
                          to=[author_email]
            )
            email.send()
        ...
        if user_email:
            email = EmailMessage(
                        "دیدگاه دریافت شد",
                         "دیدگاه شما دریافت شد و به زودی به آن پاسخ داده خواهد شد",
                          to=[user_email]
            )
            email.send()
